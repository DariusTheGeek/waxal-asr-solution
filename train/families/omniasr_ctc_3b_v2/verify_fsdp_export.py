#!/usr/bin/env python3
"""Fresh-process strict reload and forward-parity gate for an OmniASR 3B export."""
...
def main() -> int:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError(
            "OmniASR 3B strict reload requires exactly one visible CUDA device"
        )
    ...
    with tempfile.TemporaryDirectory(
        ...
        card.write_text(
            ...
            f"tokenizer: {(root / 'models/omniasr-ctc-3b-v2/omniASR_tokenizer_written_v2.model').as_posix()}\n",
            encoding="utf-8",
        )
