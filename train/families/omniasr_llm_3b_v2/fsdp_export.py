"""Distributed full-state export for a restored OmniASR LLM-3B FSDP2 model."""
...
PARITY_LANGUAGE_CODE = "lin_Latn"
PARITY_TARGET_TOKENS = (4, 5, 6)
...
def parity_input(
    device: torch.device, language_code: str = PARITY_LANGUAGE_CODE
) -> Seq2SeqBatch:
    samples = torch.linspace(
        -0.1, 0.1, 3_200, device=device, dtype=torch.float32
    ).unsqueeze(0)
    targets = torch.tensor(
        [PARITY_TARGET_TOKENS], device=device, dtype=torch.int64
    )
    return Seq2SeqBatch(
        source_seqs=samples,
...
def parity_forward(
    ...
    *,
    ...
    with torch.inference_mode(), torch.autocast(
        ...
        output = module(batch, return_logits=True)
    if not isinstance(output, tuple) or len(output) != 6:
        raise RuntimeError("LLM parity forward did not return the six-field contract")
    loss, logits, layout, _, _, _ = output
    if not isinstance(loss, Tensor) or not isinstance(logits, Tensor):
        raise RuntimeError("LLM parity forward returned non-tensor loss/logits")
    if not isinstance(layout, BatchLayout):
        raise RuntimeError("LLM parity forward returned an invalid logits layout")
...
def export_restored_fsdp_model(trainer: Any, destination: Path) -> dict[str, object]:
    ...
    if world_size != 8:
        raise RuntimeError(f"OmniASR LLM export requires exactly eight ranks, got {world_size}")
    step = int(trainer._step_nr)
    if step <= 0:
        raise RuntimeError("OmniASR LLM export requires a restored positive checkpoint step")
    destination = destination.resolve()
    if rank == 0 and destination.exists():
        raise FileExistsError(f"OmniASR LLM export destination already exists: {destination}")
    gangs.root.barrier()
    ...
    if missing:
        raise RuntimeError(f"OmniASR LLM export identity environment is incomplete: {missing}")
    ...
    if identity_payload[0] is not None:
        raise RuntimeError(f"OmniASR LLM export source identity failed: {identity_payload[0]}")
    if not isinstance(identity_payload[1], dict):
        raise RuntimeError("OmniASR LLM export source identity was not broadcast")
    source_identity = identity_payload[1]

    module = trainer._unit.model.module
    module.eval()
    parity_batch = parity_input(
        gangs.root.device, language_code=geometry.language_code
    )
    loss, logits, output_layout = parity_forward(
        module, parity_batch, device=gangs.root.device
    )
    if not bool(torch.isfinite(loss)) or not bool(torch.isfinite(logits).all()):
        raise RuntimeError("sharded parity forward produced non-finite loss/logits")
            ...
            torch.save(
                {
                    ...
                    "input_samples": 3_200,
                    "target_tokens": list(PARITY_TARGET_TOKENS),
