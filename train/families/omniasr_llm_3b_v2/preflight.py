#!/usr/bin/env python3
"""CPU-only numerical closure for the official OmniASR LLM-3B-v2 parent."""
...
# Exact strict-load count for the official v2 3B architecture.  The 3B scale
# lives in the 60-layer, 2048-wide speech encoder; the 12-layer Llama decoder
# and language-conditioned vocabulary contract remain those of the v2 family.
EXPECTED_PARAMETERS = 4_380_578_432
EXPECTED_FEATURE_EXTRACTOR_PARAMETERS = 4_210_176
EXPECTED_TRAINABLE_PARAMETERS = 4_376_368_256
...
def main() -> int:
    ...
    parent = root / "models/omniasr-llm-3b-v2/omniASR-LLM-3B-v2.pt"
    tokenizer_path = (
        root
        / "models/omniasr-llm-3b-v2/omniASR_tokenizer_written_v2.model"
    )
    verify_anchor(
        parent,
        "6c454275703c96e5f68ce3b6315345251dfc70028b1f795cd60972150bb2a044",
        17_522_679_843,
    )
    verify_anchor(
        tokenizer_path,
    ...
    with tempfile.TemporaryDirectory(prefix="waxal3-llm3b-cards-") as temporary:
        render_asset_cards(
        ...
        model = load_model(
            "waxal3_omni_llm_3b_v2_target_es_parent",
        ...
        feature_extractor_parameters = sum(
            parameter.numel()
            for parameter in model.encoder_frontend.feature_extractor.parameters()
        )
        trainable_parameters_after_recipe_freeze = (
            parameters - feature_extractor_parameters
        )
    ...
    checks = {
        ...
        "exact_feature_extractor_parameter_count": feature_extractor_parameters
        == EXPECTED_FEATURE_EXTRACTOR_PARAMETERS,
        "exact_trainable_parameter_count_after_recipe_freeze":
        trainable_parameters_after_recipe_freeze == EXPECTED_TRAINABLE_PARAMETERS,
        "encoder_layer_count": len(model.encoder.layers) == 60,
        "encoder_model_dimension": tuple(
            first_encoder_layer.self_attn.q_proj.weight.shape
        )
        == (2_048, 2_048),
        "encoder_ffn_dimension": tuple(
            first_encoder_layer.ffn.inner_proj.weight.shape
        )
        == (8_192, 2_048),
        "decoder_layer_count": len(model.llama_decoder.layers) == 12,
        ...
        "encoder_activation_checkpoint_wrappers": encoder_ac_wrappers == 60,
