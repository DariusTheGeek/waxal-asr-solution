#!/usr/bin/env python3
"""Fresh-process strict reload and forward-parity gate for an OmniASR 7B export."""
...
def main() -> int:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError(
            "OmniASR 7B strict reload requires exactly one visible CUDA device"
        )
    ...
    with tempfile.TemporaryDirectory(
        ...
        card.write_text(
            ...
            f"tokenizer: {(root / 'models/omniasr-ctc-7b-v2/omniASR_tokenizer_written_v2.model').as_posix()}\n",
            encoding="utf-8",
        )
        ...
        model.eval()
        inputs, layout = parity_input(device)
        logits, output_layout = parity_forward(
            model, inputs, layout, device=device
        )
