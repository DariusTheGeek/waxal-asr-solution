#!/usr/bin/env python3
"""CPU-only numerical closure for the untouched OmniASR CTC-7B-v2 parent."""
...
def main() -> int:
    ...
    parent = root / "models/omniasr-ctc-7b-v2/omniASR-CTC-7B-v2.pt"
    tokenizer_path = (
        root
        / "models/omniasr-ctc-7b-v2/omniASR_tokenizer_written_v2.model"
    )
    if not parent.is_file():
        raise FileNotFoundError(
            "official CTC-7B-v2 asset is not locally verified"
        )
    if parent.stat().st_size != 26_023_732_143:
        raise RuntimeError("CTC-7B-v2 official asset byte-count drift")
    verify_anchor(
        tokenizer_path,
        "8aa11a1092142ef472537476ef6e76541123e2f0d789b79f3ebd119008240b1e",
        91_481,
    )
    with tempfile.TemporaryDirectory(prefix="waxal3-ctc7b-cards-") as temporary:
        render_asset_cards(
        ...
        model = load_model(
            "waxal3_omni_ctc_7b_v2_target_es_parent",
