"""Distributed full-state export for a restored OmniASR 7B FSDP2 model."""
...
EXPECTED_PARAMETERS = 6_505_761_456
...
def export_restored_fsdp_model(trainer: Any, destination: Path) -> dict[str, object]:
    ...
    if world_size != 8:
        raise RuntimeError(f"OmniASR 7B export requires exactly eight ranks, got {world_size}")
    step = int(trainer._step_nr)
    if step <= 0:
        raise RuntimeError("OmniASR 7B export requires a restored positive checkpoint step")
    destination = destination.resolve()
    if rank == 0 and destination.exists():
        raise FileExistsError(f"OmniASR 7B export destination already exists: {destination}")
    gangs.root.barrier()
    ...
    if missing:
        raise RuntimeError(f"OmniASR 7B export identity environment is incomplete: {missing}")
    ...
    if identity_payload[0] is not None:
        raise RuntimeError(f"OmniASR 7B export source identity failed: {identity_payload[0]}")
    if not isinstance(identity_payload[1], dict):
        raise RuntimeError("OmniASR 7B export source identity was not broadcast")
