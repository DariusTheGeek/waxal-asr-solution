...
def _frozen_spec_and_profiles() -> tuple[Path, Path]:
    ...
    experiment = (
        ROOT
        / "experiments/omniasr_llm_1b_v2/mono/lin/supervised_ft"
        / "control_max12_target_es"
        / "X0030_20260804T180536674297Z_llm1b_v2_cv002_fsdp2_control"
    )
    return experiment / "experiment.yaml", experiment / "profiles"
...
def test_relocatable_llm_card_render(tmp_path: Path, monkeypatch) -> None:
    ...
    assert str(ROOT / "models/omniasr-llm-1b-v2") in text
    assert "model_family: wav2vec2_llama" in text
    assert "model_arch: 1b_v2" in text
...
def test_llm_training_and_checkpoint_contract() -> None:
    ...
    assert production["model"]["name"] == "waxal3_omni_llm_1b_v2_target_es_parent"
    ...
    for profile in (smoke, resume_smoke, production):
