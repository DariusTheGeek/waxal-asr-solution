...
def test_language_id_defaults_to_none_without_override(
    monkeypatch, tmp_path: Path
) -> None:
    manifests = tmp_path / "data/portable/lin/manifests"
    _minimal_manifests(manifests)
    monkeypatch.setenv("WAXAL3_MANIFEST_DIR", "data/portable/lin/manifests")
    monkeypatch.delenv("WAXAL3_EXPECTED_MODEL_LANGUAGE_ID", raising=False)
    geometry = runtime_geometry_from_environment(tmp_path)
    assert geometry.expected_model_language_id is None


def test_model_language_id_matches_frozen_shona_contract(tmp_path: Path) -> None:
    geometry = RuntimeGeometry(
        language="sna",
        language_code="sna_Latn",
        manifest_dir=tmp_path,
        expected_train_rows=16_293,
        updates_per_epoch=509,
        expected_model_language_id=1_343,
    )
    assert validate_model_language_id({"sna_latn": 1_343}, geometry) == 1_343


def test_model_language_id_mismatch_fails_closed(tmp_path: Path) -> None:
    geometry = RuntimeGeometry(
        language="sna",
        language_code="sna_Latn",
        manifest_dir=tmp_path,
    ...
    with pytest.raises(RuntimeError, match="model language ID drift"):
        validate_model_language_id({"sna_latn": 820}, geometry)


def test_legacy_model_language_id_accepts_positive_mapping(tmp_path: Path) -> None:
    geometry = RuntimeGeometry(
        language="lin",
        language_code="lin_Latn",
        manifest_dir=tmp_path,
        expected_train_rows=16_035,
        updates_per_epoch=501,
    )
    assert validate_model_language_id({"lin_latn": 820}, geometry) == 820
