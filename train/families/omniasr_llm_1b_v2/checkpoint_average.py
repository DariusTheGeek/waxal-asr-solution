#!/usr/bin/env python3
"""Fail-closed averaging for consolidated OmniASR LLM checkpoints.

Fairseq2 FSDP shards are deliberately rejected here: call the distributed
consolidation/export route first, then average only the resulting full states.
"""
...
def load_checkpoint(path: Path) -> object:
    if path.is_dir():
        raise CheckpointAverageError(
            "FSDP checkpoint directories require distributed consolidation before averaging"
        )
    if not path.is_file():
...
def validate_state_contract(states: Sequence[Mapping[str, Tensor]]) -> dict[str, Any]:
    ...
    if any(tensor.layout != torch.strided for state in states for tensor in state.values()):
...
def resolve_sources(
    ...
    *,
    ...
    if (
        ...
        raise CheckpointAverageError("X0030 production gate identity drift")
    ...
    if observed_steps != curve_top_steps:
        raise CheckpointAverageError(
            f"checkpoint IDs/steps {observed_steps} are not curve top three {curve_top_steps}"
        )
    gate_top = [int(value) for value in gate.get("curve", {}).get("top_five_steps", [])[:3]]
    if gate_top != observed_steps:
...
def _forward(
    ...
    *,
    ...
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise CheckpointAverageError(f"strict averaged-state load failed: {incompatible}")
    if not isinstance(model, Wav2Vec2LlamaModel):
        raise CheckpointAverageError("averaged model is not Wav2Vec2LlamaModel")
    parameters = sum(parameter.numel() for parameter in model.parameters())
    if parameters != 2_279_609_984:
        raise CheckpointAverageError(
            f"averaged model parameter drift: {parameters} != 2279609984"
        )
    model.eval()
    samples = torch.linspace(-0.1, 0.1, 3_200, dtype=torch.float32).unsqueeze(0)
    targets = torch.tensor([[4, 5, 6]], dtype=torch.int64)
    batch = Seq2SeqBatch(
        source_seqs=samples,
        source_seq_lens=[samples.shape[1]],
        target_seqs=targets,
        target_seq_lens=[targets.shape[1]],
        example={"lang": [language_code]},
    )
    with torch.inference_mode():
        loss, logits, output_layout, _, _, _ = model(batch, return_logits=True)
    ...
    if not bool(torch.isfinite(loss)):
        raise CheckpointAverageError("non-finite CPU-forward loss")
    return (
        loss.detach().clone(),
        logits.detach().clone(),
        [int(value) for value in output_layout.seq_lens],
    )
...
def build_average(
    *,
    ...
    loss_before, logits_before, lengths_before = _forward(
        averaged,
        card_dir=card_dir,
    ...
    try:
        ...
        for name in averaged:
            if not torch.equal(averaged[name], reloaded[name]):
                raise CheckpointAverageError(f"averaged tensor changed on reload: {name}")
        if tensor_content_sha256(reloaded) != content_sha256:
            raise CheckpointAverageError("averaged tensor-content hash changed on reload")
        loss_after, logits_after, lengths_after = _forward(
            reloaded,
            card_dir=card_dir,
            model_card=model_card,
            language_code=language_code,
        ...
        if (
            ...
            or not torch.equal(logits_after, logits_before)
            or not torch.equal(loss_after, loss_before)
        ):
            raise CheckpointAverageError("CPU forward changed after averaged-state reload")

        manifest = {
            "schema_version": 1,
            "status": "PASS",
            "kind": "llm_checkpoint_parameter_average",
            ...
            "cpu_forward_loss": float(loss_before),
            "language_code": language_code,
