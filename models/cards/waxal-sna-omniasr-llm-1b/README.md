...
OmniASR LLM-1B v2, fine-tuned for **Shona** as part of the
[WAXAL ASR solution](https://github.com/DariusTheGeek/waxal-asr-solution).

Released artifact: **top-3 equal-weight parameter average**. Decoded with **beam 5, length_norm=True**.
...
python models/download_models.py --repo waxal-sna-omniasr-llm-1b

.venvs/omni/bin/python inference/decode/omniasr.py \
    --config configs/sna/llm1b.yaml \
    --audio path/to/wav_dir --output transcripts.csv
```

`--weights` accepts any directory holding this repo's files, e.g. the path
returned by `huggingface_hub.snapshot_download("DariusTheGeek/waxal-sna-omniasr-llm-1b")`.
