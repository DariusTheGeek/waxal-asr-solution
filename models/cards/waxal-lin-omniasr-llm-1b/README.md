---
license: apache-2.0
base_model: facebook/omniASR-LLM-1B-v2
datasets:
- google/WaxalNLP
language:
- ln
...
OmniASR LLM-1B v2, fine-tuned for **Lingala** as part of the
[WAXAL ASR solution](https://github.com/DariusTheGeek/waxal-asr-solution).

Released artifact: **top-3 uniform FP64 parameter average**. Decoded with **beam 5, length_norm=True**.

## Files

| File | Bytes |
|---|---:|
| `model.pt` | 9,118,680,523 |
| `omniASR_tokenizer_written_v2.model` | 91,481 |
| `card.yaml` | — |
| `requirements.txt` | — |

Total weights: 9,118,680,523 bytes (9.12 GB).
...
python models/download_models.py --repo waxal-lin-omniasr-llm-1b

.venvs/omni/bin/python inference/decode/omniasr.py \
    --config configs/lin/llm1b.yaml \
    --audio path/to/wav_dir --output transcripts.csv
```

`--weights` accepts any directory holding this repo's files, e.g. the path
returned by `huggingface_hub.snapshot_download("DariusTheGeek/waxal-lin-omniasr-llm-1b")`.
...
| | |
|---|---|
| Parent | `facebook/omniASR-LLM-1B-v2` |
| Language | Lingala |
